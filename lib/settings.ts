...
let settingsPromise: Promise<UserSettings | null> | null = null;
  ...
  settingsPromise = (async () => {
    try {
      ...
      if (cachedSettings === undefined) {
        const fallback = await loadApiKeysFromLocal();
        cachedSettings = fallback ? (fallback as UserSettings) : null;
      }
      return cachedSettings;
    }
    ...
    const settings = cachedSettings as UserSettings;
    try {
      for (const field of API_KEY_FIELDS) {
        const localVal = await getItem(LOCAL_API_KEY_PREFIX + field);
        if (!settings[field] && localVal) {
          (settings as unknown as Record<string, unknown>)[field] = localVal;
        } else if (settings[field]) {
          await setItem(LOCAL_API_KEY_PREFIX + field, settings[field] as string);
        }
      }
    } catch {
      // local storage sync failed — return what we have from DB
    }
...
export async function updateUserSettings(
  ...
  try {
    ...
    // local storage sync failed — DB save succeeded, cache invalidated below
  }
