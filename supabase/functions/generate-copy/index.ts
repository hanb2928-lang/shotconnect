...
const PSYCHOLOGICAL_TRIGGERS = [
  ...
  { name: '페인 포인트 저격', desc: '유저가 일상에서 겪던 불편함을 부드럽게 짚은 뒤 이 상품이 해결 열쇠임을 각인', example: '매번 퇴근길에 거울 보며 지친 피부에 한숨 쉬셨죠? 오늘 밤 단 5분 투자로 내일 아침 결이 달라집니다' },
  { name: '소속감과 자존감 터치', desc: '단순 물건 구매가 아닌 이 물건을 쓰는 감각적이고 가치 있는 나를 상상하게 만듦', example: '이동 중에 툭 꺼내 쓰는 것만으로도 공간의 분위기가 달라지는 아이템이에요. 본인의 취향을 아시는 분들에게 꼭 어울립니다' },
...
async function generateWithOpenAI(
  ...
  const copyChannelSpecific =
    ...
    "심리 밀착형 톤: 객관적 스펙보다 유저의 내면적 갈증과 숨겨진 욕구를 자극하라. 감각적 단어(텍스처, 결, 온도, 순간)로 상상력을 자극하고, 다정하고 신뢰감 주는 대화체(~요, ~해보세요)를 사용하되 결정적 순간에는 확신에 찬 톤으로 등을 밀어줘라.\n" +
    "페인 포인트 저격: 유저가 일상에서 겪던 불편함을 부드럽게 짚은 뒤 이 상품이 해결 열쇠임을 각인시켜라. 예: '매번 퇴근길에 거울 보며 한숨 쉬셨죠? 오늘 밤 5분만 투자하면 내일 아침 결이 달라요'\n" +
    "소속감 터치: '이 물건을 쓰는 감각적이고 가치 있는 나'를 상상하게 하라. 예: '이동 중에 툭 꺼내 쓰는 것만으로 공간의 분위기가 달라지는 아이템이에요'\n" +
